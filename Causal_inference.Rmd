...
Another approach to do causal inference could be the use of a Random Forest Model. It can be use when the response is not necessary linear with respect to the predictors. However, we started to test it to see how will it work and the results obtained were not very promising. Using this model we obtained about a 15% of the variance explained, which means that the casual effect obtained will only refer to a 15% of the population of the study, which is actually a very low percentage. Therefore, we decided not to continue with this study.
